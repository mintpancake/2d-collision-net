    ...
    for i in range(n):
        ...
        for j in range(cut_size):
            for k in range(cut_size):
                # obj_grid = obj_pc_moved[(obj_pc_moved[:, 0] >= cut[j]) & (obj_pc_moved[:, 0] < cut[j+1]) &
                #                         (obj_pc_moved[:, 1] >= cut[k]) & (obj_pc_moved[:, 1] < cut[k+1])]
                # scene_grid = scene_pc[(scene_pc[:, 0] >= cut[j]) & (scene_pc[:, 0] < cut[j+1]) &
                #                       (scene_pc[:, 1] >= cut[k]) & (scene_pc[:, 1] < cut[k+1])]
                samples = np.array(
